...
2026-08-02 기준으로 그래픽 에셋에 새겨진 버튼·라벨과 별개인 폰트 렌더
문자열 418개를 `ALLBIN.BIN` unit `38`, `43`에서 추출했다. 안정 ID와 원문
...
| **합계** | **418** |  | **ROM 정적 주입 완료, 실행 검토 필요** |

이 418개에는 머신 설정 화면의 타이어·윙·부스트 설명, 그 전후의 설명·확인
분기와 코스 정보 대사가
...
| `data/translations/disc1-special-screen-ko.json` | 한국어 번역 정본 418개 | 추적 |
