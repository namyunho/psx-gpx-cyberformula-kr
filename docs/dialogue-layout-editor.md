...
표시는 한도 초과, `~` 표시는 짧은 행 후보, `*` 표시는 저장하지 않은 변경을
뜻한다.
...
결합되는 제어 셸을 다음처럼 보여준다.

```text
바이트: 원본 스트림 68B · 현재 예상 64B (안전 슬롯 한도는 별도)
...
원본 스트림 크기와 비교할 수 있지만 엔트리 뒤의 간격·무포인터 페이지까지
판정한 안전 슬롯 용량은 아니므로, 이 값만으로 삽입 가능 여부를 정하지 않는다.
