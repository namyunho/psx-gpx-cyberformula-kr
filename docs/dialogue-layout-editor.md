...
바이트 한도이자 unit별 원본 대사 스트림 총량의 기준선이다. 자세한 경계
정책은
...
검토해야 하며 빌드 차단 상태다. `u00`, `u21`은 동일 Track 1로 전체 진행
실행 검증을 통과했다. 다른 unit도 계산값은 표시하지만 전수 포인터
...
주 빌드는 모든 17×3 또는 unit 총량 위반을 실패로 처리한다. 의미를 자동
축약하거나 글리프·제어토큰 중간에서 자르지 않는다.
