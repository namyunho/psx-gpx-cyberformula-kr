...
사용자 화면 관측과 `u38/u43` 소비자 교차 분석으로 workset 422개 가운데
417개는 primary 글꼴 스트림임을 확인했다. 나머지 규칙 제목 5개는 무참조
중복 문자열이며, 실제 화면은 `MINI_G3.BIN` unit 0의 베이크드 그래픽을 쓴다.

| 화면 | 폰트 번역 범위 | 그래픽 범위 |
|---|---:|---|
| 미니게임 | 규칙 본문·블랙잭·카메라·요리 대사, 런타임 단어와 결과 합성 조각 326개 | HUD·버튼·규칙 제목 5개·타이틀 |
| 코스 정보 | 코스 설명 57개 | `Course Information`, 지도·날씨 라벨 |
| 머신 설정 | 타이어·전략·윙·부스트 설명과 순차 분기 32개 | `Machine Setting`, A/B/C Tire와 버튼 |
| 모터홈 | 행동 메뉴 2개 | 팀 이름 자막과 기타 베이크드 라벨 |
...
unit 0 child 1은 모터홈 지도에서 선택한 팀의 이름을 화면 아래에 표시하는
256×256 4bpp 공용 atlas다. Disc 1·2의 `AVM_MAP.BIN`은 SHA-256
`227cb9554f2515370aec74d4c72ffbd21c542270aed7c71db0c6b5ef039abe3c`로
동일하다. `ALLBIN.BIN + 0x1018B8..0x10195F`의 9개 SPRT descriptor가
CLUT `0x7C80`과 atlas 상단 `(U,V)=(0,0)..(0,96)`의 16px 높이 조각을
소비한다. 이 레코드로 다음 여섯 팀의 실제 조합 구조를 확인했다.

| 원문 | 번역 | 저장 조각 |
|---|---|---|
| `ミッシング リンク` | `미싱 링크` | `(0,0) 120x16` |
| `スゴウ グランプリ` | `스고 그랑프리` | `(0,16) 48x16` + `(48,16) 72x16` |
| `スゴウ ウィナーズ` | `스고 위너스` | 공유 `스고` + `(48,32) 80x16` |
| `シュトルムツェンダー` | `슈트룸 젠더` | `(0,32) 48x16` + `(0,48) 96x16` |
| `ユニオン セイバー` | `유니온 세이버` | `(0,64) 120x16` |
| `アオイ ZIP フォーミュラー` | `아오이 ZIP 포뮬러` | `(0,80) 120x16` + `(0,96) 56x16` |

삽입기는 각 조각 사각형만 투명 index 0으로 비운 뒤 Galmuri11 Regular
12px를 흰색 index 1, `(1,1)` 회색 그림자 index 4로 그린다. 16개 CLUT와
팀 아이콘, atlas의 나머지 픽셀, ALLBIN 소비자 레코드는 바꾸지 않는다.
공유 조각 경계에서는 앞 조각을 오른쪽, 뒤 조각을 왼쪽에 붙여 원래의 화면상
연결 위치를 유지한다. `슈트`+`룸`과 `포`+`뮬러`처럼 한 단어가 두 SPRT로
나뉜 경계는 여백 없이 맞붙이고, `슈트룸 젠더`의 공백은 두 번째 조각 안에
명시적으로 둔다. `스고 위너스`와 `슈트룸 젠더`는 짧은 한글 잉크가 원본의
공유 조각 경계 왼쪽에 치우쳐 보이므로, 두 팀에만 쓰이는 SPRT descriptor
네 개의 화면 X를 전각 한 칸인 `+12px` 옮긴다. 공유 atlas의 `스고` 조각은
그대로이므로 `스고 그랑프리`에는 영향이 없다. 정본은
`data/translations/disc1-motorhome-team-graphics-ko.json`, 빌드는 다음과 같다.

```bash
.venv/bin/python scripts/build_motorhome_team_graphics_patch.py \
  --file-build-dir <current-file-build> \
  --output-dir <output-file-build>
```

저장 구조와 Expected Writes 및 Disc 1·2 정적 재삽입은 검증했지만, 선택·비선택
상태의 실제 CLUT 색과 여섯 팀 전환 화면은 PCSX-Redux에서 사용자 실행 검증이
필요하다.
...
사각형으로 합쳐 총 16개 쓰기 영역이며, 모든 버튼·타입명은 원본 일본어의
실제 비투명 잉크 중심에 한글 잉크·그림자 중심을 맞춘다. 세부 위치와 빌드는
...
2026-08-08 clean boot에서 규칙 제목 캐시의 실제 저장 공급원을
`MINI_G3.BIN` unit 0 child 0으로 확정했다. child 0은 VRAM
`(768,256) 128×256 halfword`에 올라가는 512×256 4bpp atlas다. 화면에 남은
일본어 캐시 `(828,446) 68×48 halfword`와 원본 atlas 우하단 6,528바이트가
완전히 일치하며, `u38`의 동명 문자열 5개는 IDA·Ghidra에서 코드 참조가 없다.
따라서 다음 제목은 폰트 문자열 패치가 아니라 atlas 교체 대상으로 분류한다.
...
| `アンリを捕まえろ` | `앙리를 붙잡아라` |
| `ドリンク泥棒を激写` | `음료 도둑 찍기` |
| `レナの3分クッキング` | `레나의 3분 요리` |
| `ブラックジャック` | `블랙잭` |

`scripts/build_minigame_rule_graphics_patch.py`는 원본 CLUT bank 7과 cyan guard를
보존하고, 현재 primary 폰트의 흰색·회색 명암 인덱스로 다섯 슬롯만 다시
조판한다. Disc 1·2 원본 MINI_G3 동일성과 clean-boot VRAM 중첩 해시를 함께
검사한다.
...
보존한 채 다음 문구만 Galmuri11 12px로 교체한다. 저장된 56/64×16 사각형의
중앙이 아니라 각 원본 일본어 문구의 비투명 중심을 기준으로 한글 잉크·그림자
합성 중심을 맞춰 화면상의 버튼 중앙을 유지한다.
