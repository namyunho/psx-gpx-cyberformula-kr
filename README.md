...
미니게임·코스·머신 설정·관련 분기 폰트 문자열 422개와 후속 물리 gap·색인
스트림 감사에서 확인한 순차·경기·미니게임·저장 문자열 806개까지 현재 식별한
...
| 글꼴 렌더 스트림 모집단 | 🚧 확대 조사 — 직접 포인터/UI 5,843개, 무포인터 83개, 특수 화면 422개와 추가 순차·색인·저장 스트림 806개 확인; 색인 런타임 경로 전수 QA 필요 |
...
| 대사 추출 작업본 | 🚧 직접 대사 5,783개, 무포인터 83개, 이름 등록 UI 60개, 특수 화면 422개와 추가 글꼴 스트림 806개 원문 보호 기준선 확보 |
| 후보 번역 레이아웃 감사 | 🚧 직접 대사 5,783개 기계 배치 가능·차단 0 — 단어 분할 16건, 동일 원문 표기 후보 63건·용어집 후보 71건 사람 검토 필요 |
| 특수 화면 폰트 번역 | 🚧 workset 422개 중 실제 폰트 소비 417개 정적 주입 준비; 규칙 제목 중복 5개는 MINI_G3 베이크드 그래픽으로 재분류·삽입, 실행 검토 필요 |
| 추가 글꼴 스트림 번역 | 🚧 806개 초벌 번역 준비 — 새 원문 298종, 문맥 충돌 23건과 물리 미러 76개를 안정 ID로 분리, 레이아웃 초과·미매핑 0; 런타임 검수 필요 |
| 이름 등록 폰트 UI | 🚧 고정 리터럴 4개 정적 주입 완료 — 입력 팔레트·런타임 버퍼 56개 원본 보존, 실행 전수 검토 필요 |
| 전체 대사 재삽입 개발 이미지 | 🚧 `u00..u34` 직접+무포인터·추가 스트림 6,560개, 특수 화면 422개와 `u38/u39` 추가 101개 정적 주입 — 기존판은 제2장 종료까지 사용자 진행 확인, 새 통합판은 실행 검증 필요 |
| Disc 2 통합 개발 이미지 | 🚧 Disc 1과 같은 번역·폰트·UI를 Disc 2 원본 LBA에 이식, 식별 플래그 2곳 보존, 1,137개 변경 sector·EDC/ECC·최종 재추출 검증 통과; 디스크 교체와 한글 대사 출력 사용자 확인, 전편 QA 필요 |
...
> 미니게임·코스·머신 설정 및 순차 분기 422개도 분리했습니다. 이어서 기존
> 네 workset 밖의 물리 gap을 감사해 `u02..u19` 순차 페이지 295개,
> `u28`, `u30..u34` 경기 페이지 410개, `u38` 미니게임 74개와 `u39` 저장 메시지
> 27개를 추가 분리하고 번역했습니다. 경기 페이지에는 실행이 확인된 최종전
> 14페이지의 overlay별 미러 70개와 짧은 경기 미러 6개가 포함됩니다. 이 806개는 소비자 계열과
...
| 특수 화면 문자열 workset | 422개 | 실제 폰트 소비 417개와 안정 ID를 보존한 무참조 규칙 제목 중복 5개; 화면 제목은 MINI_G3 unit 0 그래픽 |
| 추가 순차·색인·저장 문자열 | 806개 | `u02..u19` 295, `u28/u30..u34` 410, `u38` 74, `u39` 27; 물리 미러 76개 포함, 59개 이진 오탐은 고정 제외 |
| 통합 폰트 편집기 | 7,059행 | 직접 대사, 무포인터·인라인 메뉴, 특수 화면, 추가 독립 항목 730개, UI 리터럴 4, 고정 이름·화자명 36을 원본별로 안전 저장; 물리 미러 76개는 각 정본 행에서 함께 생성 |
...
| [추가 순차·색인·저장 글꼴 문자열](docs/unindexed-font-text.md) | 포인터 중심 추출 밖 806개, 오탐 카탈로그, 초벌 번역과 검증 경계 |
...
| [미니게임·코스·머신 설정 폰트 문자열](docs/special-screen-font-text.md) | `u38/u43` 422개 소비자·추출·외부 AI 병합과 고정 제어 슬롯 정적 주입 |
...
요리 미니게임 버튼·말풍선, 실제 리테일 타이틀(unit 21), 11개 장 카드와 모터홈
팀명 6개와 MINI_G3 규칙 제목 5개를 한 파일 빌드에 통합했습니다. 이름 등록·
어빌리티·최종 확인, 저장 `예/아니오`, 요리 선택 버튼은 저장 사각형의 기계적
중앙이 아니라 원본 일본어 라벨의 실제 잉크 중심을 화면 합성 기준점으로 삼습니다.
최신 시험 Track 1의 SHA-256은 Disc 1
`adbde780405306f46592aae9347eb4b3854c21e1023a3b170808e3d158d69de7`, Disc 2
`c5ae02a24e19602ef98804e39920a016d54a0366a58ece5e1eb227ba9b72fae7`이며, 각각
1,137개 raw sector의 Expected Write·EDC/ECC·파일 재추출 검증을 통과했습니다.
Disc 1 빌드는 PCSX-Redux clean boot에서 편집한 한국어 타이틀이 정상 출력되는
것까지 사용자 검수를 통과했습니다. 모터홈 팀명은 `스고 위너스`와
`슈트룸 젠더`의 두 조각을 각각 `+12px` 옮기는 정렬까지 정적 삽입했으며
요리 결과를 런타임에서 이어 붙이는 짧은 문자열 4개도 번역 대상으로 편입했습니다.
미니게임의 `규칙 설명`과 게임명 4종은 무참조 u38 문자열이 아니라
`MINI_G3.BIN` unit 0의 4bpp atlas에 별도 최종 단계로 주입해 이전 빌드의
누락을 막습니다. 요리 시식·반응 19개 스트림은 원본 페이지 수와 최종 `FFFF`
하나를 보존하도록 재조립해 선행 공백·잔여 글자 원인을 제거했습니다. 선택·비선택
상태와 요리 결과 합성은 사용자 실행 검증이 필요합니다. u11 블랙잭 무작위 상대
7분기가 일반 포인터가 아닌 MIPS `LUI+ADDIU`로 발표문 주소를 직접 만들던 것도
확인해, unit 재배치 시 이 명령어 쌍을 함께 갱신하고 최종 7개 한국어 스트림과
종료 토큰을 정적 검증합니다. 무작위 7분기 전수 실행은 아직 필요합니다.
최종전에서 누락됐던 `u30..u34 +0x7190..+0x7490`의 14페이지×5 물리 미러는
`u19/ref0004..ref0017` 번역을 공유하되 각 overlay의 제어 셸과 14개 진입
참조를 독립 보존하도록 재삽입했습니다. 새 Disc 1·2 이미지에서 최종전 재현
검증이 남아 있습니다. 후속 짧은 문자열 재감사에서 확인한 순차 반응 5개와
경기 반응 미러 6개도 806개 정본에 추가했고, 같은 1,137개 raw sector 범위에서
Disc 1·2 Expected Write·EDC/ECC·파일 재추출 검증을 다시 통과했습니다.
전체 이미지는 비커밋 로컬
