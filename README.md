...
- [x] 한국어 글리프 저장 공간 및 인코딩 방식 결정
- [x] `0x5xxx` 한글 캐시 토큰 렌더러 훅 PoC 정적 빌드
- [ ] `0x5xxx` 한글 캐시 토큰 렌더러 훅 에뮬레이터 검증
- [ ] 전체 대사 추출·번역·재삽입
...
| `scripts/build_cache_hook_poc.py` | `0x5xxx` 한글 캐시 토큰 렌더러 훅 PoC 이미지 생성 |
...
적재합니다. 텍스트 스트림의 한글 토큰은 대표 텍스트 구간에서 비어 있는
`0x5xxx` 대역을 사용하고, 하위 12비트는 현재 캐시 슬롯 번호로 해석합니다.
세부 계산과 후속 PoC 목표는
[`docs/hangul-storage-encoding.md`](docs/hangul-storage-encoding.md)에 기록합니다.

`0x5xxx` 토큰을 실제 렌더러에서 가로채는 정적 훅 PoC는 `SLPS_019.58`의
`0x8003271C`에 트램폴린을 설치하고, 한글 캐시를 `0x80059800`에 둡니다. 첫
대사는 원본 슬롯 덮어쓰기 없이 `0x5000..0x5011` 캐시 토큰으로 다시 빌드됐고,
정적 디스어셈블·해시·글리프 렌더 검증을 통과했습니다. DuckStation 화면 검증은
다음 단계로 남아 있습니다. 상세 내용은
