...
미니게임·코스·머신 설정 폰트 대사 391개와 후속 물리 gap·색인 스트림
감사에서 확인한 순차·경기·미니게임·저장 문자열 716개까지 현재 식별한
...
| 대사 추출 작업본 | 🚧 직접 대사 5,783개, 무포인터 83개, 이름 등록 UI 60개, 특수 화면 391개와 추가 글꼴 스트림 716개 원문 보호 기준선 확보 |
...
> 미니게임·코스·머신 설정 391개도 분리해 정적 주입했습니다. 이어서 기존
> 네 workset 밖의 물리 gap을 감사해 `u02..u19` 순차 페이지 291개,
> `u30..u34` 경기 페이지 325개, `u38` 미니게임 73개와 `u39` 저장 메시지
> 27개를 추가 분리하고 번역·정적 주입했습니다. 이 716개는 소비자 계열과
...
| 추가 순차·색인·저장 문자열 | 716개 | `u02..u19` 291, `u30..u34` 325, `u38` 73, `u39` 27; 60개 이진 오탐은 고정 제외 |
| 통합 폰트 편집기 | 7,014행 | 직접 대사, 무포인터, 특수 화면, 추가 716개, UI 리터럴 4, 고정 이름·화자명 36을 원본별로 안전 저장 |
...
| 그래픽 제외 전체 폰트 빌드 | 위 5,866개+추가 716개+특수 화면 391개, primary 1,041자·미매핑 0 | 475개 raw sector Expected Write·EDC/ECC 검증, 새 색인·저장 경로 실행 검토 필요 |
