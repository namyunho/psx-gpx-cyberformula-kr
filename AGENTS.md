...
9. 두 디스크에 공통인 대사·폰트·UI·실행 코드·컨테이너를 바꾸면 같은 승인
   입력에서 Disc 1·2 개발 이미지를 함께 다시 만들고 각각 검증한다. 한 디스크의
   패치 파일이나 LBA를 다른 디스크에 통째로 복사하지 않으며, 각 디스크의 부트
   파일명·`START.BIN` 식별 바이트·실제 ISO LBA를 보존한다. 실행 증거도 정확한
   디스크와 Track 1 해시에 귀속하고 다른 디스크의 통과로 대신하지 않는다.
10. 새 전체 디스크 이미지가 생성되고 독립 검증까지 통과하면 `work/build/`의
    이전 생성 이미지들은 자동으로 정리한다. Disc 1·2 각각 최신 검증본 한 세트만
    남기고, 이전 Track 1 `.bin`과 그에 짝지어진 `.cue`·디스크 빌드 디렉터리를
    삭제한다. 새 빌드나 검증이 실패하면 직전 검증본을 삭제하지 않는다. 이 규칙은
